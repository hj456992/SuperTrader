...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking. The user explicitly selected five role tasks coordinated by TL; do not ask them to choose an execution mode again.
...
- [x] 阅读现有用户界面及基线，记录本人/对象两条路径。
- [x] 为成功、资料不足、无书、冲突、取消、重开各写 Given/When/Then。
- [x] 明确书籍是方法、聊天是事实、目标只影响建议的界限。
- [x] 按基线逐条审阅，不得引入新功能；提交文档并报告。
...
- [x] 读取底座 Agent/CreateAgentOptions/ToolRegistry/SessionStore 实际合同，给 TL 与运维提供插件和装配清单；若需要公共合同改造先停止该扩展并报告。
- [x] 为书籍工具结果回填后模型再次生成写失败测试，使用可控模型替身，不访问私人资料。
- [x] 实现工具作用域、材料和书籍检索、现有循环桥接，使测试通过。
- [x] 为主体串用、未知 ID、无书/无命中/预算/冲突、取消与保存竞态写失败测试并实现必要行为。
- [x] 加兼容画像字段、有限进度事件及最小任务持久化；避免 job 写入使 state revision 自增。
- [x] 运行 Maven 测试，报告确定性测试与实际运行验证的区别；提交。
...
- [x] 先用内存夹具写 Node 测试：旧画像可显示，书摘不变成人物事实，HTML 内容转义。
- [x] 增加真实阶段反馈、任务 ID 定向取消、状态隔离；保留轮询与既有视觉风格。
- [x] 展示画像知识引用、限制/反证、未知项；可选字段缺省不崩溃。
- [x] 测试快速人物切换不会显示他人的任务或书摘；取消期间不能重复启动冲突任务。
- [x] 执行 `node --test web/src/*.test.js` 及前端构建，提交并报告。
...
- [x] 基线运行 Maven 和 Node 测试，建立虚构聊天/书籍夹具。
- [x] 为主体归属、书籍来源、目标独立、循环继续、取消保存屏障、旧数据兼容建立可执行断言。
- [x] 后端完成后在自己的分支合入明确提交，跑验收；禁止修改生产代码来掩盖失败。
- [x] 报告真实失败、缺少验证的行为和确切复现命令，给 TL 复核。
...
- [x] 检查原启动器依赖与进程/端口现状，只读检查不打印环境或凭据。
- [x] 用虚构配置验证缺少制品时明确报错；按后端提供的清单补齐已有插件装配。
- [x] 确认不写盘模型密钥、不提交聊天数据，原开发实例不被擅自停止。
- [x] 准备独立端口与测试数据库的 smoke 方法、备份/回滚和重启中断检查。
- [x] 运行对应 Python 测试与构建检查，提交并报告。

### Task 6: TL 集成与交付

- [x] 记录五个角色任务 ID、分支、进度和阻塞；每次范围变化先审查。
- [x] 审核和顺序合入各角色提交，解决接口冲突；新增需求不得顺手实现。
- [x] 在同一个集成提交上运行 Java/Node/启动器测试及隔离环境 smoke。
- [x] 对关键取消链路、来源与角色边界审阅，不以 mock 通过冒充真实模型验收。
- [x] 完成本地可运行交付并明确测试范围、限制及回滚方式。
