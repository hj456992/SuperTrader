...
| 范围 | 实现与验证 | 发布／后续状态 |
|---|---|---|
| 本人／对象画像 Agent 升级 | 已实现；确定性及隔离真实模型验证完成 | `4754cb1` 已上传 GitHub，并已部署到原本地 48740 入口 |
| 本地切换与数据保护 | 已验证：成套制品、10 插件、业务数据摘要保持一致、11 项静态资源请求通过 | 已发布；保留私有备份和旧制品，未使用真实聊天作测试夹具 |
| 攻略取消、总时限与实际阶段（G01） | 已复现现有执行缺口，正在实施独立修补 | 尚未发布；沿用直接模型路径，首次调用加至多一次结构／引用修复 |
| 检索评测、来源更正、依赖与恢复 | 已完成差距拆解，功能改造待实施 | 分批选择，不并入 G01 |
| 全面攻略 Agent 化、token 流式、推荐问题、跨任务记忆 | 未实现相应扩展 | 待用户决定，不默认增加 |

[本地发布记录](docs/ops/local-release-20260925.md) · [独立发布审核](docs/qa/local-release-review-20260925.md) · [后续差距与批次](docs/product/architecture-gap-backlog-20260925.md)
