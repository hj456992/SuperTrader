...
@click.option("--structured", is_flag=True, help="授权群结构化记录（插件模式）")
@click.option("--cursor-file", type=click.Path(), help="已提交检查点输入；命令不修改此文件")
@click.pass_context
def history(ctx, chat_name, limit, offset, start_time, end_time, fmt, msg_type, media, structured, cursor_file):
    """获取指定聊天的消息记录
    ...
    """
    if structured:
        from ..core.structured_history import structured_history
        try:
            if fmt != 'json' or offset or start_time or end_time or msg_type or media:
                raise ValueError('structured mode does not accept history filters')
            result = structured_history(ctx.obj['config_path'], chat_name, cursor_file, limit)
        except Exception:
            # Never print native errors, keys, source paths or message content.
            click.echo('structured history failed: configuration, authorization, checkpoint or database validation failed', err=True)
            ctx.exit(1)
        output(result, 'json')
        return
    if cursor_file:
        raise click.UsageError('--cursor-file requires --structured')
    from ..core.context import AppContext
    app = AppContext(ctx.obj['config_path']) if isinstance(ctx.obj, dict) else ctx.obj
