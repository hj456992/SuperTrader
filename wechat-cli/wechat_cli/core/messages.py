...
# Local structured history extension. Query source and parsing remain in the
# upstream history module; only pagination/identity representation differs.
def collect_structured_history(conn, table_name, database, cursor, limit, ctx, names):
    if not _is_safe_msg_table_name(table_name):
    ...
    if not conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?", (table_name,)).fetchone():
        if cursor:
            raise ValueError('checkpoint table disappeared')
        return [], False
    maximum = int(conn.execute(f'SELECT COALESCE(MAX(local_id),0) FROM [{table_name}]').fetchone()[0])
    if cursor > maximum:
        raise ValueError('message database rolled back')
    raw_rows = conn.execute(f'''SELECT local_id, server_id, local_type, create_time,
        real_sender_id, hex(message_content), WCDB_CT_message_content,
        message_content IS NULL FROM [{table_name}]
        WHERE local_id > ? ORDER BY local_id ASC LIMIT ?''', (cursor, limit + 1)).fetchall()
    id_to_username = {int(rowid): user for rowid, user in conn.execute('SELECT rowid, user_name FROM Name2Id').fetchall()}
    display = lambda user, mapping: mapping.get(user, user)
    records = []
    for raw in raw_rows[:limit]:
        local_id, server_id, local_type, created, real_sender, content_hex, ct, is_null = raw
        local_id, local_type, created = int(local_id), int(local_type), int(created)
        content = None if int(is_null) else decompress_content(bytes.fromhex(content_hex), int(ct or 0))
        if not int(is_null) and content is None:
            raise ValueError('message decompression failed')
        sender, text = _format_message_text(local_id, local_type, content, True,
            ctx['username'], ctx['display_name'], names, display)
        real_sender = int(real_sender) if real_sender is not None else None
        sender_id = id_to_username.get(real_sender) or sender or None
        if sender_id == ctx['username']: sender_id = sender or None
        author = _resolve_sender_label(real_sender, sender, True, ctx['username'],
            ctx['display_name'], names, id_to_username, display) or '未知成员'
        records.append({'database': database, 'local_id': local_id,
            'server_id': server_id, 'sender_id': sender_id, 'author': author,
            'text': ('' if (local_type & 0xFFFFFFFF) == 1 else None) if int(is_null) else text, 'create_time': created, 'local_type': local_type})
    return records, len(raw_rows) > limit
